package com.example.study_springboots_gradle.utils;

public class Paginations {
	private int pageScale = 10; // 페이지당 게시물 수
	private int blockScale = 5; // 블록당 페이지수
	private int currentPage; // 현재 페이지 번호: client에서 넘어와야 하고, 이 class 그대로 넘어가야 한다(jsp까지)
    ...
    // 얘 호출하면 알아서 해줌. (레코드 전체 개수, client가 현재 클릭한 페이지번호)
    public Paginations(int totalCount, int currentPage){  // 생성자 
